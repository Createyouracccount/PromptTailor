...
## 2026-09-03 · R42 (실사용 로그 기반 수정: keep 출력 절약 — 약 4배 단축, v0.2.5)

- **발굴(usage.jsonl 11건)**: keep 판정이 재작성만큼 느림(30.6s·33.0s) — 모델이 keep인데도 원문 전체를 재출력
- **수정**: 3개 템플릿 "keep이면 rewritten_prompt는 빈 문자열"(원문은 엔진 보존 — 기존 코드 그대로, 빈/누락 케이스 오프라인 테스트 2건 추가). 코드 변경 없음, 템플릿+테스트만
- **실측**: keep 6·7·10s(기존 30s대), 원문 보존 3/3, 모호는 정상 rewrite. 게이트 벤치 1회 36/40=90%(모호 20/20) — 기존 90–95% 범위 내, 회귀 없음. 테스트 49/49
- **의의**: 로컬 사용 기록만으로(원문 없이) 찾은 첫 개선 — 기록→진단→수정→실측 루프의 실증
- **다음 인수 지점**: R41과 동일 + 실사용에서 keep 지연이 실제로 내려가는지 로그로 재확인

## 2026-08-31 · R41 (신규 명령 /prompt-tailor:loop — 대화의 원칙을 루프 프로토콜로, experimental)

- **배경(사용자)**: "심판이 붙는 브리프" 아티팩트(브리프 7슬롯·심판 프로토콜·래칫) 공유 + "/pm 말고 새로운 것 — 내가 말한 핵심 철학을 이식한 프롬프트로 루프를 돕는 명령" 요청·승인
- **설계 판단**: "대화에서 핵심을 뽑는" 작업은 백엔드가 불가능(인자만 받고 대화 맥락 없음 — /pm 한계로 기확인) → `:loop`는 **LLM 백엔드 무호출 순수 명령 템플릿**(추출·작성은 세션 모델 몫, 지연 0). 명령 내용: ⓪스케일 게이트(소규모→/pm 안내) ①원칙 추출(각 항목 근거 필수, 부족분은 [기본값] 표시 — 지어내기 금지) ②LOOP_LOG/GATES 생성(기존 파일 덮어쓰기 금지, 게이트=측정·임계·판정템플릿+동결조항) ③라운드 구조(검증 강도 규모 비례) ④게이트 사용자 승인 후 시작
- **검증(드라이런)**: 명령 지시를 실제 백로그 목표("게이트 벤치 영어 항목")에 그대로 수행 → PRINCIPLES(원칙 6개 전부 실제 근거 인용 + [기본값] 1)·GATES 초안(G1~G3 측정/임계/판정템플릿·래칫·동결조항)·원장 헤더 3종 산출 확인. E2E(실제 /prompt-tailor:loop 호출)는 사용자 플러그인 업데이트 후
- **부수 발견**: 이 세션에 사용자 개인 `brief` 스킬이 이미 존재(아티팩트 내용과 동일 계열) — 계획했던 `:brief`는 중복 가능성, 진행 전 사용자 확인 필요
- **README 영/한**: experimental 명시, "아직 별도 평가 전" 문구로 검증 경계 표시. 플러그인 전용 변경이라 PyPI 재배포 불필요(R37 전례)
- **다음 인수 지점**: 사용자 플러그인 업데이트 → 실제 /prompt-tailor:loop E2E, `:brief`는 기존 brief 스킬과의 관계 결정 후

## 2026-08-29 · R40 (README 데모 GIF + 데모가 잡아낸 언어 보존 회귀, v0.2.4)

- **데모(사용자 요청 "실제 동작을 직관적으로")**: VHS 테이프(docs/demo.tape) → docs/demo.gif(314KB), 두 장면 — ① 모호한 영어 요청 → 재작성 ② 이미 명확한 요청 → `"action": "keep"`, `"changes": []`. 실제 호출이며 ~20초 대기만 컷(README에 명시). 한국어 타이핑은 VHS에서 제어문자로 깨져 영어 프롬프트 채택
- **회귀 발견**: 영어 입력이 4/4 한국어로 재작성됨(concise·full 모두) — README 핵심 예시("English in → English out")가 거짓이던 상태. 원인: 메타 전체가 한국어라 "RAW 언어 유지" 한 줄이 무시됨. 3개 템플릿에 이중언어 명시 규칙 추가 → 수정 후 4/4 영어. 상세 runs/MEASUREMENT_LOG.md
- **부수 수정**: CLI `--json`에 `action` 누락(pm/MCP엔 있었음) 추가, stderr 라벨 "--- changes (변경 요약) ---"로 이중언어화, 메타 길이 불변식 800/1200 상향
- **교훈**: 문서용 데모가 곧 E2E 테스트다 — 영어 경로는 사용자가 한국어만 써서 한 번도 실측되지 않았었다. 다음 인수 지점: 게이트 벤치에 영어 항목 추가(언어별 재현율), 언어 보존을 오프라인 불변식으로 못 박을 방법(LLM 호출 없인 불가 → 벤치 항목으로)

## 2026-08-28 · R39 (실측 라운드: 속도·Windows·게이트 안정성·모호 결과 A/B — 98% 폐기, v0.2.3)

- **문서 체계(사용자 지시)**: 원시 측정은 runs/MEASUREMENT_LOG.md(추가 전용, 숫자 불변), 해석·결정은 본 원장, 확정 주장만 README — 3층 분리 확립
- **Windows CI**: windows-latest×py3.10/3.12 추가 → 4잡 전부 green(오프라인 48건). README "미검증" → "오프라인 CI 통과, E2E 미검증"으로 실측 상향
- **속도(A1)**: CLI 플래그 실험(n=3×3, 중립 cwd) — `--system-prompt` 최소 교체·`--setting-sources ''` 모두 **개선 없음(가설 기각)**: A 15.5s / B 19.2s / C 14.4s 중앙값, 분산이 차이보다 큼 → 지연은 프로세스 기동 지배. 대안으로 **옵트인 직접 API 경로** 구현(PROMPT_TAILOR_USE_API=1 + ANTHROPIC_API_KEY + `pip install 'prompt-tailor[api]'`, anthropic SDK 선택 의존성, 기본 경로 불변) — 이 기기에 키 없어 지연 미실측, README에 속도 주장 없이 문서화. 목표 "<10s"는 기본 경로 미달성
- **게이트 안정성(A3) — 98% 폐기**: 중립 cwd 3회 실측 88–90%(명확 재현율 15–16/20, 오판 전원 keep→rewrite, 상시 c02·c12). 패턴 = 짧은 자기완결 과제를 "짧다=모호"로 오판 → 게이트에 "길이가 아니라 완결성" 규칙 추가(벤치 항목 미인용) → v2 3회 **90–95%**(명확 16–18/20, 모호 59/60). 추가 수정은 과적합 위험으로 중단. README·BENCHMARK를 3회 범위로 정정하고 98%(1회·레포 cwd) 폐기를 측정 이력으로 명시. 교훈: 벤치는 중립 cwd + 3회 이상
- **모호 과제 결과 A/B(C, 핵심 실험)**: n=10(유효 8, 심판 오류 2) — **원문 4 : 재작성 4.** 모호 영역에서도 결과 우위 미입증. 승리 = 합리적 기본 구현 유도, 패배 = 조사 지시가 headless 되묻기 유발(R34 실패 모드 재현). 한계: headless 단일 턴은 확인 질문형 재작성에 구조적 불리 — 대화형 실사용 측정이 열린 과제. README "보장하지 않는 것"에 반영
- **버전**: 0.2.3 (엔진 변경: 게이트 문구 + API 경로) → 릴리스·PyPI 자동 배포
- **다음 인수 지점**: 대화형 환경 결과 측정 설계(headless 편향 제거), API 경로 지연 실측(키 보유 환경), c02·c12류 경계 사례 라벨 재검토, 실사용 stats 축적
