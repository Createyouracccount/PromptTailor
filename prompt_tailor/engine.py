...
(예: "X 파일의 Y를 Z로 바꿔줘", "퀵정렬 함수 짜줘"), action을 "keep"으로 하고 rewritten_prompt는 빈 문자열로
두어라 — 원문은 시스템이 보존하므로 다시 쓰지 마라(출력 낭비). 길이가 아니라 완결성으로 판정하라: 단일 함수·스크립트·쿼리 작성처럼 대상 동작이 완결
...
{{
  ...
  "rewritten_prompt": "재작성된 프롬프트 전문, RAW와 같은 언어로 (keep이면 빈 문자열)",
...
RAW가 이미 대상·목표·결과물이 구체적이면 action="keep", rewritten_prompt=""(원문은 시스템이 보존, 다시 쓰지 말 것) — 명확한 요청을 부풀리지 마라. 길이가 아니라 완결성으로 판정: 단일 함수·스크립트·쿼리처럼 대상 동작이 완결 지정된 요청은 짧아도 keep. 모호할 때만 action="rewrite".
...
CONDENSED_TEMPLATE = """\
당신은 프롬프트 재작성기다. RAW를 Claude {target_model}에 맞게 재작성하라.
0단계: RAW가 이미 대상·목표·결과물이 구체적이면 action="keep", rewritten_prompt=""(원문은 시스템이 보존, 다시 쓰지 말 것) — 명확한 요청을 부풀리지 마라. 길이가 아니라 완결성으로 판정: 단일 함수·스크립트·쿼리처럼 대상 동작이 완결 지정된 요청은 짧아도 keep. 모호할 때만 action="rewrite".
